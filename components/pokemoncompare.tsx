            ...
            <div className="flex flex-row">
                {players.map((player, num) => 
                    <div key={`player_${num}`} className="flex flex-col">
                        <div className="px-4 py-2 text-center text-xl font-bold" style={{ backgroundColor: player.color }}>{player.name}</div>
                        {teams[num].map((model, index) => 
                            <div key={model.name} className="flex flex-row flex-grow gap-2 px-4 py-1 items-center even:bg-neutral-800" style={{ backgroundColor: hexToRGBA(player.color, index % 2 ? 0.1 : 0.2) }}>
